...
*/
let QYWX_AM = 'wwe17047c18e582c94,HedwuU3RvW7i4BE--sGmeNHW_UYBry_ErtuKKOsjOHI,,1000002';
...
//==========================云端环境变量的判断与接收=========================
if (process.env.PUSH_KEY&&process.env.PUSH_KEY.indexOf(",")==-1) {
  SCKEY = process.env.PUSH_KEY;
}
if (process.env.PUSH_KEY&&process.env.PUSH_KEY.indexOf(",")>-1) {
  QYWX_AM = process.env.PUSH_KEY;
}
...
console.log(process.env.QYWX_KEY)
...
function fixedLengthFormatString(str,num){
    ...
    var array = new Array();
   // var len = str.length;
	var len = str.replace(/[^x00-\xff]/g, '**').length // 字节长度
    for(var i=0;i<(len/num);i++){
    	if((i+1)*num > len){
	    array.push(cutStr(str,i*num,len));
		}else{
			array.push(cutStr(str,i*num,(i+1)*num));
		}
    }
    return array;
}
var  substr=function(str, start, n) {
...
async function sendNotify(text, desp, params = {}) {
	...
	//for(var i=0;i<des.length;i++){
		await sendNotify1(text,""+des[0],params)
			if(des.length>1)
		await sendNotify1(text,"账号5"+des[1],params)
	//}*/
