...
async function filterGoodList() {
	...
	$.goodList = allGoodList.filter(good => {
		...
		// 3. good 的结束时间大于一天
		// 4. good 的价格小于最小的限制
		if (!good || good.endTime < now + 10 * 60 * 1000 || good.endTime > oneMoreDay || good.jdPrice < minPrice || good.orderPrice > 0) {
			return false
		}
		...
		$.get(taskurl(`${selfdomain}/migrate/apply?activityId=${good.id}&source=1&_s=m`, good.id), (err, resp, data) => {
			try {
				if (err) {
					...
					if (data.success) {
						$.totalTry += 1
						console.log(`🥳 ${good.id} 🛒${good.trialName.substr(0,15)}🛒 ${data.message}🥳价值 ${good.jdPrice}元 🥳共${good.supplyCount}份🥳已申请人数：${good.applyCount}`)
