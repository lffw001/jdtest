...
最少开卡一张才能邀请，自测
————————————————
入口：[ 6.1-6.18 情暖618 全民抢红包 ]

请求太频繁会被黑ip
过10分钟再执行

cron:55 13,16,20 2-18 6 *
============Quantumultx===============
[task_local]
#6.1-6.18 情暖618 全民抢红包
55 13,16,20 2-18 6 * jd_opencardL172.js, tag=6.1-6.18 情暖618 全民抢红包, enabled=true

*/

const $ = new Env('6.1-6.18 情暖618 全民抢红包');
...
!(async () => {
  ...
  $.activityId = "dz919005784fd588d3bb1641ac81ce"
  $.shareUuid = "dc03acda7d004b37bcf09b619ce6f218"
  console.log(`入口:\nhttps://lzdz1-isv.isvjcloud.com/dingzhi/highRed/active/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`)
  let shareUuidArr = ["dc03acda7d004b37bcf09b619ce6f218","","2d201ca4feb9491ead2840fd490961b2"]
  ...
  switch (type) {
    case 'isvObfuscator':
      ...
      case 'accessLogWithAD':
        ...
        let pageurl = `${domain}/dingzhi/highRed/active/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`
      ...
      case 'activityContent':
        url = `${domain}/dingzhi/highRed/active/activityContent`;
      ...
      case 'drawContent':
        ...
        break;
      case 'checkOpenCard':
        url = `${domain}/dingzhi/highRed/active/checkOpenCard`;
        body = `activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid}`
        break;
      case 'sign':
        url = `${domain}/dingzhi/highRed/active/saveTask`;
        body = `activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}&actorUuid=${$.actorUuid}&taskType=0&taskValue=0&shareUuid=${$.shareUuid}`
        break;
      case 'info':
        ...
        break;
      case 'startDraw':
        url = `${domain}/dingzhi/highRed/active/startDraw`;
        body = `activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}&actorUuid=${$.actorUuid}`
        break;
      case 'followShop':
        url = `${domain}/dingzhi/highRed/active/saveTask`;
        // url = `${domain}/dingzhi/dz/openCard/saveTask`;
        body = `activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}&actorUuid=${$.actorUuid}&taskType=23&taskValue=23`
        break;
      case 'sign':
      case 'addCart':
      case 'browseGoods':
      ...
      case 'getShareRecord':
        ...
        break;
      case 'getRule':
        url = `${domain}/dingzhi/highRed/active/getRule`;
        body = `activityId=${$.activityId}`
        break;
      case '抽奖':
        url = `${domain}/dingzhi/highRed/active/startDraw`;
        ...
        if(typeof res == 'object'){
          if(res.result && res.result === true){
            if(typeof res.data == 'object'){
              ...
              if(type == 'followShop'){
                title = '关注'
                if(res.data.beanNumStatus && res.data.beanNum){
                  msg += ` 额外获得:${res.data.beanNum}京豆`
                }
                ...
                msg = res.data.drawOk == true && (res.data.drawInfoType == 6 && res.data.name || '') || res.data.wdsrvon.drawOk == true && (res.data.wdsrvo.drawInfoType == 6 && res.data.wdsrvo.name || '') || '空气💨'
              }
              if(!msg){
                msg = '空气💨'
              }
...
function getPostRequest(url, body, method="POST") {
  ...
  if(url.indexOf('https://lzdz1-isv.isvjcloud.com') > -1){
    headers["Referer"] = `https://lzdz1-isv.isvjcloud.com/dingzhi/highRed/active/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`
...
function getCk() {
  return new Promise(resolve => {
    let get = {
      url:`https://lzdz1-isv.isvjcloud.com/dingzhi/highRed/active/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`,
