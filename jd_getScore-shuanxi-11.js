...
let stderr = fs.createWriteStream('./shuanxi.log', options);
		...
		$.get(option, (err, resp, data) => {
			try {
				if (err) {
					...
					//console.log(data)
					console.log("当前账号sessionId："+cookie)
					data = JSON.parse(data)
					//console.log(data)
					if (data.return_code==0) {//成功
					    var res = JSON.parse(data.result);
						console.log(res.points);
						logger.log("当前账号sessionId"+cookie+",积分:"+res.points);
...
function getSession(){
	...
	var amx=chars.length;
	var pwd="springSession=";
	for(var i=0;i<len;i++){
		pwd+=chars.charAt(Math.floor(Math.random()*amx));
		if(i==11||i==15||i==19){
			pwd+="-"
		}
	}
	//pwd="springSession=4e29dada-d2ff-402f-ab1f-f7e993849733"
	return pwd;
}
// prettier-ignore
