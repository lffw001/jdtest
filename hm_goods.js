...
!(async () => {
	//await notify.sendNotify(``, "测试");return;
		do{
			...
			if(data1&&data1.data.list.length>0){
					console.log("话费有货了----------------")
						var str="陕西好猫有货了`, `陕西好猫 话费有货了！！！！！";
						for(var i=0;i<data1.data.list.length;i++){
							str=str+"\n"+data1.data.list[i].goodsName;
						}
						await notify.sendNotify(str);
					delay=60*1000*30;
					setTimeout(function(){delay=1000;},60*1000*60)
			}else{
						console.log("话费没货！！！")
			}
			data1=null
